...
import { NewBulkOrderForm } from "@/components/orders/NewBulkOrderForm";
...
export default async function NewBulkOrderPage() {
  ...
  if (error) console.error("[NewBulkOrderPage] menu fetch error", error);
  ...
  return (
    <div>
      ...
      <div className="sticky top-0 z-10 bg-background border-b border-sidebar-border px-6 py-5 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-foreground">New Bulk Order</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Pre-scheduled — fulfilled on the selected Saturday
          </p>
        </div>
      </div>
      ...
      <div className="px-6 pt-[100px] pb-6">
        <div className="max-w-2xl mx-auto">
          <NewBulkOrderForm menuItems={menuItems ?? []} />
        </div>
      </div>
    </div>
  );
